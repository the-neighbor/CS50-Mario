...
function Map:collides(tile)
   -- checks if a tile value passed in as a parameter is collidable or not
   local collidables = {
      TILE_BRICK, JUMP_BLOCK, JUMP_BLOCK_HIT,
      MUSHROOM_TOP, MUSHROOM_BOTTOM
   }
   -- create a table containing all of the values for solid tiles

   -- iterate through them using a for loop to try and match the input tile to each one
   for _, v in ipairs(collidables) do
      if tile.id == v then
         -- if the tile matches a value in the table, return true
         return true
      end
   end
   -- if we get to the end of the for loop without any matches, return false
   return false
end
...
function Map:tileAt(x, y)
  ...
  -- returns a table of the tile info containing the tile id and the tilegrid coordinates
end
