...
function Player:resetPosition()
    -- reset back to the starting position so we can start a new level
...
function Player:is_flag(tile)
	--check if a given tile is a flag tile
	if tile.id == FLAG_BOTTOM or
		tile.id == FLAG_MIDDLE or
		tile.id == FLAG_TOP then
		return true
	else
		return false
	end
...
function Player:check_win()
	--check if any of the four corners of the bounding box intersect with a flag tile
...
function Player:update(dt)
	...
	if self:check_win() then
		--if we touch the flag, generate a new level
		self.map:new_level()
	end
    -- apply velocity
